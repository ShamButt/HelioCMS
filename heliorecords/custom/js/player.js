...
;(function($){
	...
	$(document).ready(function(){
		...
		if(  !$.cookie('playing-status') ){
			$.cookie('playing-status', "playing",{ expires: 360 * 10 , path: "/" });
		}
		...
		var save_current_play_status = function( status ) {
			...
			$.cookie('playing-status', status ? "playing" : "paused",{ expires: 360 * 10 , path: "/" });
		};
        ...
        if(typeof $.cookie('noo-playlist') === 'undefined' ){
            ...
            if(_songs){
				...
				var playing_status = $.cookie('playing-status');
        ...
        $("#home_play_music").ready(function(e){
            var _songs = $("#home_play_music").data('songs');
		...
		/* ############## */
		$('.team-detail').each(function(){
			var p = $(this).find('p.ds');
			var divh = $(this).height() - $(this).find('.team_socials').height() - $(this).find('.more_btn').height() - 100;
			console.log($(this).height() + " -- " + $(this).find('.team_socials').height());
